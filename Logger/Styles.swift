//
//  Styles.swift
//  Logger
//
//  Created by Anton Kononenko on 9/18/21.
//

import SwiftUI

extension Color {
    static func backgroundColor() -> Color {
        Color(red: 36 / 255, green: 32 / 255, blue: 37 / 255)
    }
    
    static func backgroundColorBar() -> Color {
        Color(red: 45 / 255, green: 41 / 255, blue: 46 / 255)
    }
    
    static func backgroundColorSeparatorLine() -> Color {
        Color(red: 58 / 255, green: 54 / 255, blue: 58 / 255)
    }
    
    static func fontLightGrayColor() -> Color {
        Color(red: 182 / 255, green: 179 / 255, blue: 182 / 255)
    }
    
    static func fontColor() -> Color {
        Color(red: 235 / 255, green: 235 / 255, blue: 235 / 255)
    }
}


